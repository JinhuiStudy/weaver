# ADR-006 — Free-tier First 정책 (운영 $0 + 유저 $0)

- **상태**: 수락 (2026-04-21) · **2026-04-22 개정**: "유저 비용 $0" 조항 추가 (ADR-007 연계)
- **Supersedes**: ADR-002 (Durable Objects) — 무료 대안으로 전환
- **관련**: TECH_STACK.md, ARCHITECTURE.md, **ADR-007 (Evolving Agent Network)**

## 맥락

Weaver MVP는 **개인이 자력으로 런칭**하는 프로젝트. 수익 발생 전까지 고정 운영 비용이 발생하면 재정적 스트레스로 개발 지속성이 깨진다.

ADR-007 피봇 (2026-04-22) 에서 **유저도 비용 $0** 으로 강화. "영원히 무료" 가 핵심 차별점.

## 결정

**두 축 모두 $0 을 유지한다**:
1. **운영 비용 $0** — Weaver 가 Cloudflare/Axiom 등에 내는 고정 월 비용 없음
2. **유저 비용 $0** — 유저가 Weaver 를 쓰는 데 어떤 외부 과금도 요구하지 않음 (BYOK 는 "선택 옵션", 기본은 Workers AI 공유)

### 원칙

1. **고정 월 비용 $0** — 런칭까지 단 한 달도 결제 카드 등록 없이 동작해야 함
2. **자체 호스팅 1:1 대안 필수** — 모든 유료 SaaS 의존 시 OSS self-host 경로 문서화
3. **유저 $0 원칙** — 회원가입·실행·fork·구독까지 **모든 기본 기능이 0 달러**. Workers AI (Llama 3.3) 를 기본 LLM 으로 공유 (유저당 일 cap).
4. **BYOK 는 옵션** — 유저가 원할 때만 Claude/GPT 키 입력. 기본 플로우에서 BYOK 요구 금지.
5. **Paid 전환은 트리거 기반** — 월 활성 유저 · 에러 수 · trace 량이 무료 한도에 도달할 때 의사결정
6. **개인 개발 도구 ≠ 프로젝트 비용** — Claude Code 구독, GitHub Copilot 등 박진희 개인 계정 도구는 이 정책 밖. 프로젝트 월 비용 $0 는 Weaver 인프라만 해당.
...
| 프런트 | **Cloudflare Workers (RR7 Framework Mode)** | 100k req/day | DAU 5k+ |
| 메타 DB | Cloudflare D1 | 5GB · 5M reads/day · 100k writes/day | 툴 10만 개+ |
| 파일 | Cloudflare R2 | 10GB · 1M Class A ops | 파일 100만 개+ |
| KV | Cloudflare KV | 100k reads/day · 1k writes/day | 세션 폭증 |
| 벡터 검색 | **Cloudflare Vectorize** (ADR-007에서 도입) | 30M queried · 5M stored dims/월 | 임베딩 수백만+ |
| 카운터 | Cloudflare Analytics Engine | 100k writes/day | trending 폭증 |
| 추론 (기본) | Workers AI (Llama 3.3) | 10k neurons/day **공유** (유저당 cap) | 상업 배포 → BYOK 권장 |
| 추론 (고품질, 선택) | 유저 BYOK | 유저가 원하는 경우만, 유저 부담 | — |
| Trace | Axiom Free | 500GB/월 | 초과 시 → 자체 호스팅 Jaeger |
| 에러 | Sentry Developer | 5k errors/월 | 초과 시 → GlitchTip 자체 호스팅 |
| 이메일 (선택) | Resend Free | 3,000 mail/월 | Magic link 폭증 시 MailChannels |
| 시크릿 관리 | **Doppler Free** + `wrangler secret` | 5 user · 3 config | 팀 증가 시 Team plan |
| CWV | Cloudflare Web Analytics | 무료 | — |
| CI | GitHub Actions (private/public) | 2000분/월 private, 무제한 public | — |
| 도메인 | `*.jinhuistudy.workers.dev` | 무료 서브도메인 | Week 13+ 런칭 시 `.dev` 구매 판단 |
